...
\begin{sectionbox}
	...
	\\
	$\hat{\theta}_{ML} = \underset{θ ∈ Θ}{\argmax} \{ f_{\vec{X},\vec{Y}}(\vec{x},\vec{y};\theta) \} $\\
	\\
	Express the maximum likelihood (ML) estimator $\hat{\theta}_{ML}$ for $\theta$ in terms of k and d(x, y).
	\textbf{Hint}: You may assume that a stationary point is a maximizer:\\
	\\
	$ g(\vec{x},\vec{y},\theta) \Big\rvert_{ \theta = \hat{\theta}_{ML} } = 0 $\\
	\\
...
\boxed{$$
\begin{array}{l}
\mathrm{E}\left[X_{1}\right]=g_{1,1} \mathrm{E}\left[X_{0}\right]+\mathrm{E}\left[V_{1}\right]=0 \\
\mathrm{E}\left[X_{2}\right]=g_{2,1} \mathrm{E}\left[X_{1}\right]+g_{2,2} \mathrm{E}\left[X_{0}\right]+\mathrm{E}\left[V_{2}\right]=0
\end{array}
$$}
\end{sectionbox}
